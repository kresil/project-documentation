...
In contrast to the electrical circuit breaker which needs a manual reset, the circuit breaker mechanism automatically resets itself after a predefined period of time.
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.9\textwidth]{../figures/05_circuit-breaker-state-machine}
...
\begin{table}[!htb]
    ...
    \begin{tabular}{|p{5cm}|p{5cm}|p{6cm}|}
        ...
        \texttt{failureRateThreshold} & \texttt{0.5} & The rate in percentage of calls
        recorded as failures
        that will trigger the circuit breaker to transition to the \texttt{Open} state if equaled or exceeded. \\ \hline
        \texttt{permittedNumberOfCalls InHalfOpenState} & \texttt{10} & The number of calls
        allowed in the \texttt{HalfOpen} state. \\ \hline
        \texttt{maxWaitDurationInHalf OpenState} & \texttt{0} & The maximum duration the circuit breaker will wait in the \texttt{HalfOpen} state before transitioning to the \texttt{Closed} state.
        ...
        & Configures the sliding window used to record calls and calculate the failure rate. \\ \hline
...
Originally,
the state component of the circuit breaker was implemented using a regular finite state machine (FSM)~\cite{fsm}.
But upon further analysis,
it was decided that the state component should be designed to be potentially shared among multiple circuit breakers.
Allowing multiple circuit breakers, with possibly different configurations,
to share the same state component can be useful in scenarios
where the circuit breakers are protecting related operations.
Because in a system, if one or more related components fail, the others are likely to fail too.

The circuit breaker was implemented
using a state reducer pattern~\cite{redux-state-reducer} to manage the state transitions.
The state reducer pattern is a design pattern
that allows the state of a system to be managed by a single reducer function that takes the current state and an action as input and returns the new state.

This implementation was designed to be inline with the principles of the Redux~\cite{redux} state management library, which is widely used in the JavaScript ecosystem.

TODO: talk about how only the reducer needs to use a lock (mutex) to ensure thread safety
dispatch events to the reducer in a thread-safe manner

TODO: talk about internal events and how they are used to trigger state transitions

\subsection{Sliding Window}\label{subsec:cbreaker-sliding-window}

TODO: talk about ring buffer and other things that were to implement the count-based sliding window (time-based is not implemented yet)
