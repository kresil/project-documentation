...
\begin{itemize}
    ...
    \item \textbf{Last Replinishment Period}: The timestamp of the last time the permits were replenished;
...
\begin{boldenumerate}
    ...
    \item The other node, upon receiving the message, removes the first (corresponding) request from its local queue.
    ...
    As the request was processed by the same node, it doesn't need to alert itself,
    and as such, the (corresponding) request is only removed from its local queue.
\end{boldenumerate}
